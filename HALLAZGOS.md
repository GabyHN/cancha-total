...
| 5 | 3.1 — Se cancela hasta 24 horas antes de la hora del partido | Compara solo fechas: cualquier reserva de mañana se puede cancelar aunque falten menos de 24 horas (mañana 8:00 se cancela hoy a las 23:00), y ninguna de hoy se puede cancelar nunca. | comportamiento | `pruebas/suite.test.js::con menos de 24 horas de anticipación ya no se puede cancelar` | **cerrado** — commit «Comportamiento: el plazo de cancelación se mide contra la hora del partido»; la prueba pasa sin la marca de fallo esperado y sin haberla modificado |
...
`verificar.sh` termina en **0**. Los hallazgos #1, #2, #3, #4 y #5 están cerrados: sus
pruebas pasan sin marca. No queda ningún hallazgo de comportamiento abierto. La prueba del
hallazgo #5 se omite sola cuando el reloj no permite construir el caso (corre de 09:00 en
adelante); esa limitación es del hallazgo de estructura #8, que sigue abierto.
